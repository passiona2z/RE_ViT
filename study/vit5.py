...
def train(vit, trainloader, criterion, optimizer, scheduler, device, clip_value):
    vit.train()
    running_loss = 0.0
    for images, labels in trainloader:
        ...
        optimizer.step()
        scheduler.step()  # Update the scheduler for each batch
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        scheduler = LinearLR(optimizer, start_factor=1.0, end_factor=0.0, total_iters=total_iters)

        best_acc = 0
        for epoch in range(args.epochs):
            train_loss = train(vit, trainloader, criterion, optimizer, scheduler, device, args.clip_value)
            ...
            if val_acc > best_acc:
                ...
                torch.save(vit.state_dict(), './model5.pth')

    else:
        vit.load_state_dict(torch.load('./model5.pth'))
