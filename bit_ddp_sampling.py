...
def parse_args():
    ...
    parser.add_argument("--dataset_ex", type=bool, default=False)
    parser.add_argument("--dataset_ratio", type=int, default=1.0)
...
def main():
    ...
    if args.dataset_ex == True :
        dataset["train"] = dataset["train"].train_test_split(train_size=args.dataset_ratio, seed=args.seed)["train"]
        if accelerator.is_main_process:
            print(f'adjust dataset : {args.dataset_ratio} - train_dataset {len(dataset["train"])}')
    ...
    if accelerator.is_main_process :
        accelerator.save_state(args.save_dir+args.model_name+"_D_"+args.dataset_name+"_S"+str(args.dataset_ratio)+"R_"+str(args.resolution))
