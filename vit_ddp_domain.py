...
def parse_args():
    ...
    parser.add_argument(
        "--resolution", type=int, default=384)          # x%16 = 0, 384(서브메인)
    parser.add_argument(
        "--batch_size", type=int, default=32)           # per GPU : 16*4 / 32*2 : 총 배치 :64/128 
    parser.add_argument(
        "--save_dir", type=str, default="./saved/")    
    parser.add_argument(
        "--epochs", type=int, default=7)                # 총 에포크                  
...
# 허깅페이스 -  Accelerator(멀티GPU) - (파이토치) dataloader
